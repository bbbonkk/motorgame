...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private float minGrabDistance = 1.5f;
    [SerializeField] private float maxGrabDistance = 10f;
    [SerializeField] private float scrollSensitivity = 0.005f;
    [SerializeField] private bool invertScroll = true; // NEW: Toggle to reel object in when scrolling up
    ...
    private void HandleRaycast()
    {
        ...
        // ===== HIGHLIGHT LOGIC =====
        if (hitSomething && grabbedObject == null)
        {
        ...
        if (Mouse.current.leftButton.wasPressedThisFrame && hitSomething)
        {
            ...
            if (rb != null)
            {
                ...
                grabDistance = Vector3.Distance(cameraTransform.position, grabbedObject.position);
        ...
        {
            // --- UPDATED: Scroll wheel logic to reel object closer ---
            float scrollY = Mouse.current.scroll.ReadValue().y;
            if (Mathf.Abs(scrollY) > 0.01f)
            {
                // Subtracting the scroll value pulls it closer when scrolling up
                if (invertScroll)
                {
                    grabDistance -= scrollY * scrollSensitivity;
                }
                else
                {
                    grabDistance += scrollY * scrollSensitivity;
